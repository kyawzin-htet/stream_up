...
function resolveFromHeader(value) {
    if (!value)
        return 'en';
    const lower = value.toLowerCase();
    if (lower.includes('mm') || lower.includes('my'))
        return 'mm';
    return 'en';
}
function resolveLanguage(request) {
    ...
    if (typeof explicit === 'string')
        return resolveFromHeader(explicit);
    if (Array.isArray(explicit) && explicit[0])
        return resolveFromHeader(explicit[0]);
    const accept = request.headers['accept-language'];
    if (typeof accept === 'string')
        return resolveFromHeader(accept);
    if (Array.isArray(accept) && accept[0])
        return resolveFromHeader(accept[0]);
    return 'en';
}
function localizeSingle(message, language) {
    if (language === 'en')
        return message;
    if (MM_MAP[message])
        return MM_MAP[message];
    if (message.includes('must be an email'))
        return 'Email ပုံစံမှန်ကန်ရပါမည်။';
    if (message.includes('must be longer than or equal to'))
        return 'စာသားအရှည် မလုံလောက်ပါ။';
    if (message.includes('should not be empty'))
        return 'အလွတ်မဖြစ်ရပါ။';
    if (message.includes('must be a string'))
        return 'စာသားအမျိုးအစား ဖြစ်ရပါမည်။';
    if (message.includes('must be a boolean'))
        return 'boolean တန်ဖိုး ဖြစ်ရပါမည်။';
    return message;
}
function localizeMessage(message, language) {
